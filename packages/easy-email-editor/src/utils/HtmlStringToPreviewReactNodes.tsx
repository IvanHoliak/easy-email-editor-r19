...
import { getNodeTypeFromClassName } from '@ivanholiak/easy-email-core';
...
const RenderReactNode = React.memo(function ({
  ...
  if (node.nodeType === Node.TEXT_NODE) {
    ...
    // Skip whitespace-only text nodes to avoid React 19 hydration warnings
    if (text.trim() === '') {
      return <></>;
    }
    return <>{text}</>;
  ...
  if (node.nodeType === Node.ELEMENT_NODE) {
    ...
    // В React 19 нельзя рендерить html/head/body как React элементы
    // Используем div вместо них для предотвращения ошибок
    const actualTagName = ['html', 'head', 'body'].includes(tagName) ? 'div' : tagName;

    const blockType = getNodeTypeFromClassName(node.classList);

    if (attributes['data-contenteditable'] === 'true') {
      return React.createElement(actualTagName, {
        key: performance.now(),
        ...attributes,
    ...
    const reactNode = React.createElement(actualTagName, {
      key: index,
      ...attributes,
      style: getStyle(node.getAttribute('style')),
      children:
